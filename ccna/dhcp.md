...
Se debe excluir aquellas direcciones que estén configuradas manualmente en algun router, impresora, servidor, etc. Se ejecuta el siguiente comando en el modo de configuración global:
...
Ejecutar el siguiente comando para entrar en el modo de configuración del grupo. Dentro del modo de configuración global:
...
````
1)network {network-address} [mask | prefix-length]
...
Para deshabilitar el servicio, usar el comando **no service dhcp** en el modo de configuración global.

Para volver activar el servidor -> **service dhcp**

## Retransmisión tráfico DHCP a un servidor DHCP

El tráfico que usa un cliente DHCP es broadcast y los router no reenvía tráfico broadcast. Para que un cliente puede usar un servidor DHCP que está fuera de su red, se configura en la interfaz del router conectada a la red de los clientes DHCP. Dentro del modo de configuración de la interfaz usar este comando:
